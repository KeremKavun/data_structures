...
{
    if (deallocator)
        walk_ht(hs, userdata, deallocator);
    free(hs->items);
...
static int resize(struct hash_set* hs, float factor)
{
    ...
    // Could not reuse insert_ht code because objects pointed to by key value pairs are already owned by this hash_set
    ...
    for (size_t i = 0; i < old_capacity; i++)
    {
        ...
        {
            int attempts = 0;
            int index = hs->hash(get_key(curr_item), hs->capacity, attempts);
            void** curr_slot = &hs->items[index];
            while (!is_null(curr_slot))
            {
                attempts++;
                index = hs->hash(get_key(curr_item), hs->capacity, attempts);
